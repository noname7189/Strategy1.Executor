...
namespace Strategy1.Executor.Core.Converter
{
    internal class DateTimeConverter : JsonConverter
    {
            ...
            if (reader.TokenType == JsonToken.String)
            {
                ...
                if (text.Length == 8)
                {
                    if (
                        ...
                        || !int.TryParse(text.Substring(4, 2), out var result2)
                        || !int.TryParse(text.Substring(6, 2), out var result3)
                    )
                    {
                        Trace.WriteLine(
                            $"{DateTime.Now:yyyy/MM/dd HH:mm:ss:fff} | Warning | Unknown DateTime format: "
                                + reader.Value
                        );
                        return null;
                    }
                ...
                {
                    if (
                        !int.TryParse(text.Substring(0, 2), out var result4)
                        || !int.TryParse(text.Substring(2, 2), out var result5)
                        || !int.TryParse(text.Substring(4, 2), out var result6)
                    )
                    {
                        Trace.WriteLine(
                            "{DateTime.Now:yyyy/MM/dd HH:mm:ss:fff} | Warning | Unknown DateTime format: "
                ...
                if (text.Length == 10)
                {
                    ...
                    if (
                        !int.TryParse(array[0], out var result8)
                        || !int.TryParse(array[1], out var result9)
                        || !int.TryParse(array[2], out var result10)
                    )
                    {
                        Trace.WriteLine(
                            "{DateTime.Now:yyyy/MM/dd HH:mm:ss:fff} | Warning | Unknown DateTime format: "
        ...
        public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
        {
            ...
            {
                writer.WriteValue(
                    (long)Math.Round(((DateTime)value - new DateTime(1970, 1, 1)).TotalMilliseconds)
                );
            }
        }
    }
